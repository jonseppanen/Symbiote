...
checkForDesktopSwitcher()
{
    Global desktopSwitcherOpened
    if(WinGetClass("A") = "MultitaskingViewFrame")
    {
        dockHide()
        desktopSwitcherOpened := 1
    }
    else if(desktopSwitcherOpened)
    {
        desktopSwitcherOpened := 0
        Sleep 300
        dockShow()
    }
}
...
{   
    dockConfig["edge"] := (iconTheme["hFull"] + 15)
    dockConfig["axis"] := (iconTheme["wFull"])
    dockConfig["y"] := (A_ScreenHeight - (iconTheme["hFull"] * 4))
...
else
{
    ...
    dockConfig["axis"] := (iconTheme["wFull"])
    dockConfig["y"] := -(iconTheme["hFull"] * 1)
    dockConfig["x"] := ((A_ScreenWidth / 2) - (iconTheme["wFull"] * 5))
}  
...
SetTimer("dockStateHandler", 150)
SetTimerAndFire("eventLoop",100)
;SetTimer("checkForDesktopSwitcher", 150)



createTaskObject(taskRef, targetArray, stackArray)
{
    ...
    if(!stackArray[OutFileName])
    {
        ClassIndex := 1
    }
    else
    {
        ClassIndex := stackArray[OutFileName] + 1
    } 

    stackArray[OutFileName] := ClassIndex
    ...
    targetArray[A_Index,"classIndex"] := ClassIndex
...
{    
    ...
    {
        createTaskObject(A_LoopField,arrayTasksCheck,arrayTasksStack)
    }
    ...
    For TaskId in arrayTasksCheck
    {
        ...
        {            
            renderMeter(arrayTasksCheck[TaskId],A_Index,arrayTasksStack[arrayTasksCheck[TaskId,"exeext"]])
            ;renderMeter(arrayTasksCheck[TaskId],A_Index,ClassCounter[ClassString])
        }
    ...
    if(arrayTasks.length() != arrayTasksCheck.length())
    {
        if(arrayTasks.length() > arrayTasksCheck.length())
        {
            Loop (arrayTasks.length() - arrayTasksCheck.length())
            {
                ...
                SendRainmeterCommand("!HideMeter Indicator" .  (A_Index + arrayTasksCheck.length()) . " ")
            }
        }
